import numpy as np
from problem_settings import ProblemInputs, DataSegmenter
from utils import normalize
from abc import ABC, abstractmethod


class DataRetriever:
    def __init__(self, data_segmenter: DataSegmenter) -> None:
        self.data_segmenter = data_segmenter

    @abstractmethod
    def get_data(self, window_id: int) -> ProblemInputs:
        pass
...
class MMSEDataRetriever(DataRetriever):
    ...
    def mmse_generate_non_stationary_inputs(
        ...
        Delta = (
            Delta * np.linalg.norm(A_0, "fro") * diff_var / np.linalg.norm(Delta, "fro")
        )
        noise = rng.normal(
        ...
        weights = self.weight_function(nb_windows)
