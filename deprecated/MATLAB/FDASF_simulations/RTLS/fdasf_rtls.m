function [x,rho_track,norm_track,norm_star_track]=fdasf_rtls(params,data,conv,debug,X_star)
...
while (tol_rho>0 && abs(rho-rho_old)>tol_rho) || (i<nbiter)
    ...
    XGnew=comp_XG(Ryyc,rydc,rdd,LLc,Compressor,delta,rho);
...
function [XG,nb_ind]=comp_XG(Ryyc,rydc,rdd,LLc,Compressor,delta,rho)

    XG=rtls_aux_solver(Ryyc,rydc,rdd,LLc,Compressor,delta,rho);
