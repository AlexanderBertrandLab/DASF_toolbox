...
for k in range(mc_runs):
    ...
    # Create adjacency matrix of a random graph.
    ...
    solver_params = {'tol_f': 1e-12, 'maxiter': 300}
    X_star = tro.tro_solver(prob_params, data, X0=None, solver_params=solver_params)
    ...
    # Dictionary related to stopping conditions. We fix the number of iterations the DASF algorithm will perform to 300.
    nbiter = 300
    conv = {'nbiter': nbiter}

    # Solve the TRO in a distributed way using the DASF algorithm.
    solver_params = {'tol_X': 1e-8, 'maxiter': 10}
    X_est, norm_diff, norm_err, f_seq = dasf(prob_params, data, tro.tro_solver,
                                             conv, solver_params, tro.tro_select_sol, tro.tro_eval)

    norm_error.append(norm_err)

    # Solve the TRO in a distributed way using the F-DASF algorithm.
    X_est_fdasf, norm_diff_fdasf, norm_err_fdasf, f_seq_fdasf = dasf(
        prob_params, data, tro.tro_aux_solver, conv, solver_params, tro.tro_select_sol, tro.tro_eval)
