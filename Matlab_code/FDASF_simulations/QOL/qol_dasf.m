function [x,rho_track,norm_star_track,iter_count_vec]=qol_dasf(params,data,conv,debug,X_star)
...
while (tol_rho>0 && abs(rho-rho_old)>tol_rho) || (i<nbiter)
    ...
    [XGnew,iter_count]=comp_XG(Ryycompressed,Bcompressed,Ccompressed,d);
...
function [XG,iter_count]=comp_XG(Ryycompressed,Bcompressed,Ccompressed,d)

    [XG,iter_count,~]=qol_dinkelbach(Ryycompressed,Bcompressed,Ccompressed,d);
