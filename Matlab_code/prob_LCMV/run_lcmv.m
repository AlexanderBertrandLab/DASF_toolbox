% Example script to run the DSFO algorithm to solve the Linearly
% Constrained Minimum Variance (LCMV) Problem.
...
while n_runs<=mc_runs
    ...
    % Create the data.
    [Y,B,H]=create_data(nbsensors_vec,nbnodes,nbsamples,Q);
    ...
    [X_star,f_star]=lcmv_solver(prob_params,data);
    ...
    % Solve the LCMV in a distributed way using the DSFO framework.
    [X_est,norm_diff,norm_err,f_seq]=dsfo(prob_params,data,...
                @lcmv_solver,conv,[],@lcmv_eval);
...
function [Y,B,H]=create_data(nbsensors_vec,nbnodes,nbsamples,Q)
...
function [Y,A]=create_signal(nbsamples,nbsensors_vec,nbnodes)
