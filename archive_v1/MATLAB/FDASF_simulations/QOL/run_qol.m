% Example script to run the F-DASF algorithm on the quadratic over linear
% problem
...
nbnodes=10;
nbsensors_vec=10*ones(1,nbnodes);
nbsamples=10000;
...
for n_runs=1:mc_runs
    ...
    [Y,B,C,d]=create_data(nbsensors,nbsamples,Q);
    ...
    % Centralized QOL problem used as ground truth, solved in 3 different
    % ways:
    % - Using analytical solution
    [X_star,rho_star]=qol_solver(Ryy,B,C,d);
    % - Using a generic solver
    [X_star_2,rho_star_2]=qol_solver_2(Ryy,B,C,d,Q);
    % - Using Dinkelbach's procedure
    [X_star_3,~,rho_star_3]=qol_dinkelbach(Ryy,B,C,d);
    ...
    [X_fdasf,rho_track_fdasf,error_track_fdasf]=...
        qol_fdasf(params,data,conv,debug,X_star);
    [X_dasf,rho_track_dasf,error_track_dasf,iter_count_vec]=...
        qol_dasf(params,data,conv,debug,X_star);
...
ylabel('MSE $\epsilon$','Interpreter','latex','FontSize',20)
